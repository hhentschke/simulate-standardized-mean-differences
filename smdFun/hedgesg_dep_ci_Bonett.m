function ci=hedgesg_dep_ci_Bonett(es,s1,s2,n1,n2,xyCorr,xyCov,tCrit,zCrit,ncPar)
% ** function ci=hedgesg_dep_ci_Bonett(es,s1,s2,n1,n2,xyCorr,xyCov,tCrit,zCrit,ncPar)
% computes ci of Hedges's g for dependent data described by Bonett (2015)
...
% - se (formula 9)
se=sqrt(es.^2.*(s1.^2 + s2.^2 + 2*xyCorr.^2.*s1.*s2)./(8*df*((s1+s2)/2).^2) ...
  + sD./((s1+s2)/2.*df));
